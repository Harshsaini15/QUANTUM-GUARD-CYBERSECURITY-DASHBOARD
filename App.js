...
function App() {
  ...
  const [loadingStep, setLoadingStep] = useState(0); // For PDF pipeline steps
  const [file, setFile] = useState(null);
  const [selectedDataset, setSelectedDataset] = useState("");

  // Pipeline messages from the PDF methodology
  const pipeline = [
  ...
  const handleAnalysis = async () => {
    setLoadingStep(1);
    // Cycle through PDF methodology steps visually
    ...
    try {
      ...
      clearInterval(interval);
      setLoadingStep(0);
      alert("Backend Error: Start server.js on Port 5001");
    }
  };
        ...
        {view === 'home' && (
          <>
            ...
            <section id="workflow-section" className="workflow-section">
                ...
                <p style={{color:'var(--text-dim)', marginTop:'10px'}}>Hybrid Quantum-Classical sequential pipeline</p>
          ...
          <section style={{paddingTop:'120px'}}>
             ...
             <div className="charts-grid">
                ...
                <div className="glass-card">
                    ...
                    <Bar data={{labels:['Duration','Src Bytes','Dst Bytes','Flags'], datasets:[{label:'Chi-Square Score', data:[95,85,71,60], backgroundColor:'#3b82f6', borderRadius:8}]}} options={{plugins:{legend:{display:false}}}} />
                </div>
             </div>
             <div className="glass-card mt-30 p-0">
                <h4 style={{padding:'20px 30px 0'}}>Fidelity Kernel Matrix Snapshot</h4>
                <table>
                    <thead><tr><th>Record ID</th><th>Source Port</th><th>Dest Port</th><th>Prediction</th><th>Confidence</th></tr></thead>
                    <tbody>
                        <tr><td>WMN-102</td><td>443</td><td>80</td><td><span className="badge-red">BLACKHOLE</span></td><td>94%</td></tr>
                        <tr><td>WMN-103</td><td>8080</td><td>443</td><td><span className="badge-green">NORMAL</span></td><td>88%</td></tr>
                        <tr><td>WMN-104</td><td>22</td><td>22</td><td><span className="badge-red">JELLYFISH</span></td><td>100%</td></tr>
                    </tbody>
                </table>
             </div>
          </section>
